...
import com.viatom.bluetooth.ble.callback.OnBleConnectResult;
import com.viatom.bluetooth.ble.listener.BleConnectionStateListener;
import com.viatom.bluetooth.ble.listener.GetInfoBleListener;
import com.viatom.bluetooth.ble.listener.ParaSyncBleListener;
import com.viatom.bluetooth.ble.listener.PingBleListener;
import com.viatom.bluetooth.ble.listener.ReadBleFileListener;
import com.viatom.bluetooth.classic.BTConstant;
import com.viatom.bluetooth.classic.cmd.EndReadAckPkg;
import com.viatom.bluetooth.classic.cmd.EndReadPkg;
import com.viatom.bluetooth.classic.cmd.GetInfoAckPkg;
import com.viatom.bluetooth.classic.cmd.GetInfoPkg;
import com.viatom.bluetooth.classic.cmd.ParaSyncAckPkg;
import com.viatom.bluetooth.classic.cmd.ParaSyncPkg;
import com.viatom.bluetooth.classic.cmd.ReadContentAckPkg;
import com.viatom.bluetooth.classic.cmd.ReadContentPkg;
import com.viatom.bluetooth.classic.cmd.StartReadAckPkg;
import com.viatom.bluetooth.classic.cmd.StartReadPkg;
import com.viatom.bluetooth.utils.StringUtils;
import com.viatom.update.SharedPrefHelper;
...
public class BlePresenter implements OnBleConnectResult{
    ...
    private void onCommandReceive(byte[] bytes) {
        if(currentRequestId == BLE_REQUEST_ID_PING) {
            ...
            if(start == com.viatom.bluetooth.classic.BTConstant.COMMON_ACK_PKG_LENGTH) {
                com.viatom.bluetooth.classic.cmd.PingAckPkg pingAckPkg = new com.viatom.bluetooth.classic.cmd.PingAckPkg(tmpData);
                byte cmd = pingAckPkg.getCmd();
                byte[] datBuf = pingAckPkg.getBuf();
                if(cmd == com.viatom.bluetooth.classic.BTConstant.ACK_CMD_OK&&datBuf[0]==0x55) {
                    // ping已成功返回
            ...
            System.arraycopy(bytes,0, tmpData, start, bytes.length );
            start = start + bytes.length;
            if(start == com.viatom.bluetooth.classic.BTConstant.GET_INFO_ACK_PKG_LENGTH) {
                com.viatom.bluetooth.classic.cmd.GetInfoAckPkg getInfoAckPkg = new GetInfoAckPkg(tmpData);
            ...
            start = start + bytes.length;
            if(start == com.viatom.bluetooth.classic.BTConstant.COMMON_ACK_PKG_LENGTH) {
                com.viatom.bluetooth.classic.cmd.ParaSyncAckPkg paraSyncAckPkg = new ParaSyncAckPkg(tmpData);
                if (paraSyncAckPkg.getCmd() == com.viatom.bluetooth.classic.BTConstant.ACK_CMD_OK) {
                    currentRequestId = BLE_REQUEST_ID_AVAILABLE;
            ...
            start = start + bytes.length;
            if (start == com.viatom.bluetooth.classic.BTConstant.COMMON_ACK_PKG_LENGTH) {
                com.viatom.bluetooth.classic.cmd.StartReadAckPkg sraPkg = new StartReadAckPkg(tmpData);
                byte cmd = sraPkg.getCmd();
                if (cmd == com.viatom.bluetooth.classic.BTConstant.ACK_CMD_OK) {
                    if (sraPkg.getFileSize()<=0) {
                        ...
                        if(num == 1) {
                            ...
                            start = 0;
                            readFileContent(curPkgNum, com.viatom.bluetooth.classic.BTConstant.READ_CONTENT_ACK_DATA_LENGTH_BLE
                                    + com.viatom.bluetooth.classic.BTConstant.READ_CONTENT_ACK_PKG_FRONT_LENGTH);
                        }
                    }
                } else {
                    if(mReadBleFileListener != null) {
            ...
            if (start == curPkaSize) {
                com.viatom.bluetooth.classic.cmd.ReadContentAckPkg rcap = new ReadContentAckPkg(tmpData);
                byte[] dataBuf = rcap.getDataBuf();
                if(rcap.getCmd() != com.viatom.bluetooth.classic.BTConstant.ACK_CMD_OK){
                    ...
                    int curPos = curPkgNum * com.viatom.bluetooth.classic.BTConstant.READ_CONTENT_ACK_DATA_LENGTH_BLE;
            ...
            start = start + bytes.length;
            if (start == com.viatom.bluetooth.classic.BTConstant.COMMON_ACK_PKG_LENGTH) {
                com.viatom.bluetooth.classic.cmd.EndReadAckPkg erap = new EndReadAckPkg(tmpData);
    ...
    public void interfacePing(PingBleListener pingBleListener){
        ...
        btHandler.post(() -> {
            com.viatom.bluetooth.classic.cmd.PingPkg pkg = new com.viatom.bluetooth.classic.cmd.PingPkg();
    ...
    public void interfaceGetInfo(GetInfoBleListener getInfoBleListener){
        ...
        btHandler.post(() -> {
            com.viatom.bluetooth.classic.cmd.GetInfoPkg pkg = new GetInfoPkg();
            Single<byte[]> single = mRxBleHelper.writeCharacteristic(mRxBleConnection, UUID.fromString(writeUUID), pkg.getBuf());
            if(single != null) {
                Disposable disposable = single.subscribe(
                        bytes -> {
    ...
    public void interfaceParaSync(ParaSyncBleListener paraSyncBleListener){
        ...
        btHandler.post(() -> {
            ...
            com.viatom.bluetooth.classic.cmd.ParaSyncPkg paraSyncPkg = new ParaSyncPkg(objectSetTime);
    ...
    public void interfaceReadFile(String fileName, byte fileType, ReadBleFileListener listener) {
        ...
        btHandler.post(() -> {
            com.viatom.bluetooth.classic.cmd.StartReadPkg startReadPkg = new StartReadPkg(fileName);
    ...
    private void setCurrentReadFileInfo(int fileSize) {
        ...
        num = fileSize/ com.viatom.bluetooth.classic.BTConstant.READ_CONTENT_ACK_DATA_LENGTH_BLE;
        lastPkgBytes = fileSize % com.viatom.bluetooth.classic.BTConstant.READ_CONTENT_ACK_DATA_LENGTH_BLE;
        num += (lastPkgBytes == 0) ? 0 : 1;
        lastPkgBytes += com.viatom.bluetooth.classic.BTConstant.READ_CONTENT_ACK_PKG_FRONT_LENGTH;
    }
    ...
    private void readFileContent(int pkgNum, int wantBytes) {
        ...
        btHandler.post(() -> {
            com.viatom.bluetooth.classic.cmd.ReadContentPkg pkg = new ReadContentPkg(pkgNum);
            Single<byte[]> single = mRxBleHelper.writeCharacteristic(mRxBleConnection, UUID.fromString(writeUUID), pkg.getBuf());
            if(single != null) {
                Disposable disposable = single.subscribe(
                        bytes -> {
    ...
    private void endFileRead() {
        ...
        btHandler.post(() -> {
            com.viatom.bluetooth.classic.cmd.EndReadPkg pkg = new EndReadPkg();
            Single<byte[]> single = mRxBleHelper.writeCharacteristic(mRxBleConnection, UUID.fromString(writeUUID), pkg.getBuf());
            if(single != null) {
                Disposable disposable = single.subscribe(
                        bytes -> {
